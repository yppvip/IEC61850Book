...
## 已确认的追加功能：信号描述、便捷阅帧与趋势图

### 目标

- 导入 SCD/ICD 后，在已成功映射的 GOOSE/SV 信号旁展示可追溯的中文/工程描述；地址（FCDA 引用）仍须保留，描述不能替代地址或映射证据。
- 将帧列表与帧详情在宽屏下并排展示，使选择帧后的字段、原始字节和映射证据无需在两个远离的区块间来回滚动；窄屏保持单列，并提供选择后滚动到详情的无障碍跳转。
- 在信号时间线中勾选可绘制信号，离线绘制其随抓包时间变化的趋势。状态量使用阶梯图，数值测量量使用折线/散点图；字符串、时间、原始字节、未知/不安全解码值不提供选择或绘制。

### 可行性与前提

- 三项功能均可在当前纯前端、`file://` 可用的架构中完成，不需要网络、上传、服务器或第三方图表库；绘图以 SVG/Canvas 和本地 JavaScript 实现。
- SCD/ICD **不保证**包含人类可读的业务描述。工具只能展示有明确来源的描述，例如实例或类型模板上的 `desc`、可解析的 `EnumType/EnumVal` 文本、以及可验证关联到该 FCDA 的工程对象描述；不得由 IED 名、LN 类名、APPID、MAC 或厂商名称猜测“实际含义”。
- 若工程文件没有相应 `desc` 或模型不完整，描述单元格直接留空，不显示“未提供描述”等占位文本；FCDA 地址、类型路径和映射状态仍独立展示。若项目需要“间隔/一次设备/保护动作”的业务化名称，需提供含这些描述的完整 SCD，或另行定义并导入已授权的点表映射文件；该外部点表不在本次基础范围内。
- 趋势图的可信前提是“帧内值已按 FCDA 和数据类型可靠解码”。现有 SV 仅保留整个 `sample` 原始字节，尚未形成逐通道数值；因此在证据不足、采样布局/比例/质量定义不明确或存在多个 SCL 候选时，只保留原始值和诊断，不可绘图。

### 数据模型与解析方案

1. 将 SCL 解析结果从纯字符串 FCDA 标签扩展为结构化条目：稳定 `signalKey`（IED/LD/LN/DO/DA/FC 路径）、显示地址、`fc`、`desc`、`descriptionSource`、类型链、枚举映射、比例/单位证据和候选/冲突信息。
2. 按确定的优先级收集描述：FCDA/实例直接描述优先；其次是解析到同一数据路径的 DO/DA/类型模板描述；枚举仅用于解释已确认的枚举值。每条显示的描述都记录 XML 路径或来源类别，多个不一致描述列为候选而非自动合并。
3. GOOSE `allData` 继续按已唯一匹配的 DataSet/FCDA 顺序逐项绑定，并为解码结果保存 `valueKind`（boolean、integer、unsigned、float、enum、string、timestamp、raw 等）、原始 TLV、质量和可绘制状态。
4. SV 先以 `APPID + svID + DataSet + SampledValueControl` 唯一确定发布流，再按 FCDA 与已证实的样本编码/通道宽度拆分 `sample`。仅当通道数量、字节宽度、符号/浮点编码、质量项和比例均有工程证据时生成单通道数值；否则提示“样本布局待确认”，不将字节猜作测量量。
5. 对数值工程换算分列保存 `rawValue` 与 `engineeringValue`。没有明确比例、单位或质量定义时，图表只能使用并标注原始数值，或直接禁用绘制；不得伪造工程量。

### 交互与 UI 方案

- 新增“帧工作区”容器：左侧为带协议筛选的可滚动帧列表，右侧为粘性帧详情；选择行需高亮、更新 `aria-selected`，详情顶部提供“上一帧/下一帧”和当前帧号。移动端改回纵向布局，选择后将焦点/视图移至详情标题。
- 帧详情除字段树和十六进制外，增加“信号映射证据”表：地址、描述、描述来源、值类型、原始/工程值、质量、SCL 候选与冲突原因。未映射或描述缺失同样明确展示。
- 信号时间线每行增加可选复选框（不可绘制行禁用并说明原因）、地址/描述列或可展开的辅助文本；没有工程描述时描述单元格保持为空。筛选仍按地址、描述、值、协议、APPID 和映射状态工作。
- 选中的 `signalKey` 汇总为按时间排序的序列。ST/布尔/已确认枚举用离散阶梯图，MX 的已确认数值用连续折线并标出实际抓包时间戳；不连接超过可配置阈值的时间间隙，并保留来源帧可回选。
- 图表支持移除已选信号、时间范围筛选、悬停显示时间/值/质量/帧号及与时间线互相跳转。多量纲或不同单位不强行共用纵轴，应分面板或使用独立纵轴，并在图例中显示原始/工程值状态。

### 实施步骤

1. 为现有原创 GOOSE/SV 最小夹具补充含/不含 `desc`、类型模板、枚举、冲突与未知 sample 布局的 SCL 用例；先写出预期描述来源、可绘制性和降级结果。
2. 重构 `parseScl` 与映射结果为结构化信号语义，完成描述/类型链/枚举的可追溯解析，并保持原有 FCDA 文本显示与冲突不自动选择的行为。
3. 扩展 GOOSE 值类型解码及 SV 的证据驱动逐通道解码；增加 `valueKind`、原始值、工程值、质量和 `plotEligibility`，对不支持类型始终安全降级。
4. 调整 `packet-parser.html` 和 `style.css` 为响应式帧工作区，补充详情映射证据、键盘操作和移动端跳转。
5. 在时间线加入选择状态与本地 SVG/Canvas 趋势图，实施状态阶梯图、数值图、时间筛选、提示信息及帧联动。
6. 验证 `file://`、无 SCL、无描述、多个候选、字符串/时间/原始样本禁选、截断帧、大时间跨度和多单位信号等场景；核对不引入网络依赖且所有现有入口有效。

### 追加验收标准

- 已导入的 SCD/ICD 含可关联描述时，时间线与详情显示描述、地址和描述来源；描述缺失时描述单元格为空，描述冲突时不伪造业务含义并给出映射冲突原因。
- 宽屏中帧列表与详情可并排工作，选择帧即可查看详情；窄屏和键盘操作仍可用。
- 仅唯一映射且可可靠数值化的 ST/MX 信号可选择绘图；字符串、时间、原始字节、未知类型、映射冲突和样本布局不明的信号不可选并说明原因。
- 每个图点可追溯到抓包时间、原始/工程值、质量和来源帧；图表不改变或持久化原始抓包数据。
